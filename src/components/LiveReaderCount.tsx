...
export default function LiveReaderCount({ postId, compact = false, viewCount = 0 }: LiveReaderCountProps) {
  const { t } = useLanguage();
    ...
    return (
      <div className="flex items-center gap-2 text-[10px] font-inter font-bold">
        ...
        <span className="dark:text-white/60">
          <span className="font-black">{displayViews(viewCount, postId)}</span> {t("views", "दृश्य")}
        </span>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2 border border-black/10 dark:border-white/10">
      ...
      <span className="text-sm font-inter dark:text-white">
        <span className="font-black text-lg">{displayViews(viewCount, postId).toLocaleString()}</span>
