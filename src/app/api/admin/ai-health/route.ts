...
export async function GET(req: NextRequest) {
  ...
  const [gemini, geminiSearch, claude] = await Promise.all([
    checkGemini(),
    checkGeminiSearch(),
    checkClaude(),
  ]);

  const allOk = gemini.ok && geminiSearch.ok && claude.ok;
  const anyAi = gemini.ok || claude.ok;
  const canGenerate = anyAi && geminiSearch.ok;

  return NextResponse.json({
    ok: allOk,
    canGenerate,
    summary: canGenerate
      ? (allOk ? "All providers healthy" : "Generation will work, but some providers are degraded")
      : "Generation will fail — at least one of Gemini/Claude needs to work, and Gemini Search must work for topic discovery",
    providers: {
      "gemini (writing)": gemini,
      "gemini search (topic discovery)": geminiSearch,
